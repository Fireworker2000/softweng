...
public class Timer {
	...
	public void checkIncrement() {
		...
		if (((this.ctr.memory.readRegisterDirect(0x81) & 0x20) >> 5) == 0x01) {
			// RA0
			if (((this.ctr.memory.readRegisterDirect(0x81) & 0x10) >> 4) == 0x01) {
				if (ra4edge == 2) {
					this.increment();
				}
	...
	private void increment() {
		this.prescaler++;
		int psa = (((this.ctr.memory.readRegisterDirect(0x81) >> 3) & 0x01));
		int psavalue = (this.ctr.memory.readRegisterDirect(0x81) & 0x07);
		System.out.println("PSV" + psavalue);
		int tmr0 = (this.ctr.memory.readRegisterDirect(0x01));
		// 2 hoch Value * 2
		// PSA0 TMR0, PSA1 WD
		if ((psa == 0 && prescaler == Math.pow(2.0, psavalue) * 2) || psa == 1) {
			...
			// Tmr0 inc
			{
				...
				ctr.memory.clrTimer();
				ctr.memory.checkZFlag(0);
				ctr.memory.setRegisterBit(0x0B, 2, 1);
